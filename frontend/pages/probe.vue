...
<template>
  <main class="page">
    ...
    <div v-if="store.probe" class="grid">
      ...
      <section class="panel"><StatusBar label="燃料" :value="store.probe.fuel" /></section>
      ...
      <section class="panel">
        ...
        <p v-if="!Object.keys(store.probe.collected_resources).length" class="muted">未収集</p>
      </section>
      <section class="panel">
        <h2>発見数</h2>
        <p>天体 {{ store.probe.discovered_body_ids.length }}</p>
        <p>恒星系 {{ store.systems.length }}</p>
      </section>
    </div>
  </main>
</template>
