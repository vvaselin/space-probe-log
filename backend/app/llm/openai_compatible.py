...
def normalize_generated_log_payload(data: dict[str, Any], fallback_summary: str) -> dict[str, Any]:
    return {
        "title": data.get("title") or "INSOMNIA-07 航行記録",
...
class OpenAICompatibleLLMClient:
    ...
    async def generate_log(self, context: LogContext) -> GeneratedLog:
        ...
        data = await self._complete_json(
            ...
            "Write like a fictional exploration blog, not a dry report. Use OVIS's first-person inner monologue where appropriate. "
            "Avoid merely repeating the action summary. "
        ...
        try:
            ...
            return GeneratedLog(
                title="INSOMNIA-07 航行記録",
                summary=context.event["summary"],
                body_markdown=(
                    "## 確認済みの事実\n"
                    "ログ生成応答が不正だったため、新規の観測事実は本文へ追加しません。\n\n"
                    "## OVISの解釈\n"
                    "通信文の整形に失敗しました。記録の信頼度を下げて保存します。\n\n"
